...
export function createPartSelection(viewport,canvas,camera,getModel,onSelect){
 ...
 let selected=null,hover=null,lastModel=null,start=null;
 function sync(){const model=getModel();if(model!==lastModel){scene.clear();lastModel=model;if(model)model.traverse(m=>{if(m.isMesh){const copy=new THREE.Mesh(m.geometry,depth);copy.matrixAutoUpdate=false;copy.userData.original=m;scene.add(copy);}});}model?.updateMatrixWorld(true);for(const copy of scene.children){copy.matrix.copy(copy.userData.original.matrixWorld);copy.visible=true;}}
 function draw(){sync();overlay.clear();const id=hover??selected;if(id==null||!lastModel)return;scene.overrideMaterial=depth;overlay.render(scene,camera);scene.overrideMaterial=tint;for(const copy of scene.children)copy.visible=String(copy.userData.original.userData.regionId)===String(id);overlay.render(scene,camera);scene.overrideMaterial=null;}
