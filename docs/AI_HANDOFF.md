...
GitHub Pages currently also runs the repository legacy branch deployment. The custom Vite deploy waits via workflow_run for `pages build and deployment` to finish, then publishes dist. Do not restore competing push-triggered deployments: the legacy job can overwrite dist with unbuilt source. When repository Pages settings are switched exclusively to GitHub Actions, switch this workflow back to push.
