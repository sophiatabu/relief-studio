...
В `public/achievement-library/` добавлены 79 реальных SVG-а́чивок из пользовательской папки. Имена файлов сохранены, чтобы библиотека оставалась пригодной для поиска и последующего подключения к панели примеров.
